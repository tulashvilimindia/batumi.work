/**
 * JobDetail Component - V5 Enhanced Folk Edition
 * Full job detail view with warm traditional styling and similar jobs
 */

import React from 'react';
import { useParams, Link } from 'react-router-dom';
import { ArrowLeft, ExternalLink, Heart, Share2, Star, Send } from 'lucide-react';
      ...
      >
        ...
        <footer className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            {/* V5: Quick Apply Button */}
            <a
              href={job.source_url}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg font-semibold text-sm transition-all duration-200 hover:scale-105"
              style={{
        ...
        {/* V5: Telegram Alerts Link */}
        <div
          className="mt-6 p-4 rounded-lg flex items-center justify-between gap-4"
          style={{
            background: 'linear-gradient(135deg, rgba(45, 90, 61, 0.1), rgba(74, 157, 109, 0.1))',
        ...
        >
          ...
          <a
            href="https://t.me/batumi_work_bot"
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 px-3 py-2 rounded-lg transition-all hover:scale-105"
            style={{
              background: 'linear-gradient(135deg, #2D5A3D, #3D7A5D)',
              border: '1px solid #4A9D6D',
            }}
          >
            <Send size={14} style={{ color: '#F5E6D3' }} />
            <span
